...
> 스프링 빈은 컨테이너의 시작과 함께 생성되어서 스프링 컨테이너가 종료될 때까지 유지된다.

> 스프링 빈이 기본적으로 싱글톤 스코프로 생성된다

> 스코프틑 번역 그래도 빈이 존재할 수 있는 범위를 뜻한다
...
- 프로토타입 : 스프링 컨테이너는 프로토타입 빈의 생성과 의존관계 주입까지만 관여하고 더는 관리하지 않는 매우 작은 범위의 스코프이다
